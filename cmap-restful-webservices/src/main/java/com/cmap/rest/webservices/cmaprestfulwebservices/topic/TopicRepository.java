...
public interface TopicRepository extends JpaRepository<TopicEntity, String> {
	@Query(value = "SELECT std_guidelines.name, std_ka.title, std_ku.title, std_topic.topic_code, std_topic.title, std_topic.level, std_topic.requirement \r\n"
	...
	@Query(value = "SELECT std_guidelines.name, std_ka.title, std_ku.title, rmit_course_topic.topic_code, std_topic.title, rmit_course_topic.level, rmit_course_topic.requirement\r\n"
